...
import { validateCommentConfig } from './config.mjs';
import { getMatchingSnippetIds } from './snippets.mjs';
import {
  assembleCommentBody,
  extractCommentKey,
...
async function run() {
  ...
  if (commentKey) {
    debug(`Input comment-key was passed: ${commentKey}`);
  } else {
    debug('Input comment-key was not passed');
  }
